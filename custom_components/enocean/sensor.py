...
SENSOR_DESC_CO2 = SensorEntityDescription(
    key=SENSOR_TYPE_CO2,
    name="CO2",
...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend(
    {
        ...
        vol.Optional(CONF_EEP, default=""): cv.string,
    }
)
...
def setup_platform(hass, config, add_entities, discovery_info=None):
    ...
    if sensor_type == SENSOR_TYPE_TEMPERATURE:
        ...
        eep = config[CONF_EEP]
        entities = [EnOceanHumiditySensor(dev_id, dev_name, SENSOR_DESC_HUMIDITY, eep = eep)]
        ...
        entities = [EnOceanCO2Sensor(dev_id, dev_name, SENSOR_DESC_CO2)]
...
class EnOceanTemperatureSensor(EnOceanSensor):
    ...
    def value_changed(self, packet):
        ...
        if self._eep:
            eep = self._eep.split("-")
            packet.parse_eep(int(eep[1], 16), int(eep[2], 16))
            self._attr_native_value = round(packet.parsed["TMP"]["value"], 1)
            self.schedule_update_ha_state()
            return
...
class EnOceanHumiditySensor(EnOceanSensor):
    ...
    def value_changed(self, packet):
        ...
        if self._eep:
            eep = self._eep.split("-")
            packet.parse_eep(int(eep[1], 16), int(eep[2], 16))
            self._attr_native_value = round(packet.parsed["HUM"]["value"], 1)
            self.schedule_update_ha_state()
            return
...
class EnOceanCO2Sensor(EnOceanSensor):
    """Representation of an EnOcean CO2 sensor device."

    EEPs (EnOcean Equipment Profiles):
    - A5-09-08 (A5-09-08 Pure CO2 Sensor)
    - A5-09-09 (A5-09-09 Pure CO2 Sensor with Power Failure Detection)
    """
    ...
    def value_changed(self, packet):
        ...
        self._attr_native_value = int(packet.data[3] / 255 * 2000)
        self.schedule_update_ha_state()
